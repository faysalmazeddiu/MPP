...
public class GuestListJava8 {
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		// implement
		List<String> namesOfExpected = invited.stream().filter(y -> {
			try {
				return checkLegal(y);
			} catch (IllegalGuestException ex) {
				System.out.println("IllegalGuestException");
				throw new RuntimeException(ex);
			}
		}).filter(g -> g.getGender() == Gender.F && g.isPlanningToCome()).map(x -> x.getName())
				.collect(Collectors.toList());

		Collections.sort(namesOfExpected);
		String guests = namesOfExpected.toString();
		guests = guests.substring(1, guests.length() - 1);
		System.out.println(guests + " are invited");
	}
