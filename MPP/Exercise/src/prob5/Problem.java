...
public class Problem {
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		//implement
		List<String> listOfString=list.stream().filter(c->c.getCity().length()>=6 && !c.getCity().contains("e"))
				.map(c->c.getZip()).distinct().sorted().collect(Collectors.toList());
		return listOfString;
	}
