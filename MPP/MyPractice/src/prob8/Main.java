...
//	
	private boolean findProduct(String prodName) {
		for(OrderItem item : orderItems) {
			Optional<OrderItem> optionalItem = Optional.ofNullable(item);
			if(optionalItem.isPresent()) {
				Optional<Product> p = Optional.ofNullable(optionalItem.get().getProduct());
				if(p.isPresent()) {
					Optional<String> name = Optional.ofNullable(p.get().getProductName());
					if(name.isPresent()) {
						if(name.get().equals(prodName)) return true;
					}
				}
			}
		}
