...
public class Main {
	...
	public static void main(String[] args) {
		...
		/*
			List<String> returnList=list.stream().filter(cus->cus.getCity().equals("Fairfield"))
					.map(cus->cus.getName())
		...
		List<String> returnList = list.stream()
				.filter(LibraryComapnion::getSpecificCustomerByCity)
				.map(LibraryComapnion::separateNameFromeCustomer)
				.sorted(LibraryComapnion::customerCompairing).collect(Collectors.toList());
		System.out.println(returnList);
